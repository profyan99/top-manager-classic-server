...
const endGame = async (game: Game, em: EntityManager) => {
  ...
  const winner = actualPlayers.reduce((winner, currentPlayer) =>
  ...
  const userUpdates = actualPlayers.map(async player => {
    ...
    currentUser.gameStats.hoursInGame +=
      (game.periodDuration * game.maxPeriods) / 3600;
  ...
  actualPlayers.forEach(player =>
    sendPlayerUpdate(game, player, game.currentPeriod),
  );

  logger.info(`Game ${game.name}[${game.id}]: was ended. ${winner.companyName}
  with ${winner.getCompanyByPeriod(game.currentPeriod).rating} rating win!`);
};
