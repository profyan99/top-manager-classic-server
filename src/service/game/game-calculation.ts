...
const getBuyersForCompany = (
  company: Company,
  period: GamePeriod,
  scenario: Scenario,
  industryBuyersAmount: number,
) => {
  let nir = 0;
  let marketing = 0;

  const price =
    (industryBuyersAmount *
      scenario.sharePriceEffect *
      Math.pow(1 / company.price, 3)) /
    period.totalPrice;

  if (period.summaryNir > 0) {
    nir =
      (industryBuyersAmount * scenario.shareNirEffect * company.sumNir) /
      period.summaryNir;
  }
  ...
  if (period.totalMarketing > 0) {
    marketing =
      (industryBuyersAmount *
        scenario.shareMarketingEffect *
        Math.pow(company.marketing / company.price, 1.5)) /
      period.totalMarketing;
  }
...
const calculateGame = async (game: Game, em: EntityManager) => {
  ...
  logger.info(
    `Game ${game.name}[${game.id}]: calculating ${game.currentPeriod} period`,
  );
  ...
  companies.forEach(player => {
    ...
    const companyDataPrevious: Company = player.getCompanyByPeriod(
    ...
    period.totalMarketing += Math.pow(
      companyData.marketing / companyData.price,
      1.5,
    );
    period.totalPrice += Math.pow(1 / companyData.price, 3);

    companyData.sumNir = companyDataPrevious.sumNir;
    companyData.maxPredictedSales =
      companyDataPrevious.storage + companyData.production;
    companyData.maxPredictedRevenue =
      companyData.price * companyData.maxPredictedSales;
  });
  ...
  period.summaryPeriodMarketing =
    sumMarketing > 16800
      ? Math.sqrt(sumMarketing / 4 + 12600)
      : Math.sqrt(sumMarketing);
  ...
  if (period.averagePeriodPrice > 40) {
    period.totalBuyers = Math.round(
      (period.totalBuyers * 40) / period.averagePeriodPrice,
    );
  }
  ...
  for (const player of companies) {
    ...
    const companyBuyers: number = getBuyersForCompany(
      company,
      period,
      scenario,
      period.totalBuyers,
  ...
  if (buyersForRichPrice > 0) {
    for (const player of companies) {
      ...
      const additionalCompanyBuyers: number = getBuyersForCompany(
        company,
        period,
        scenario,
        buyersForRichPrice,
  ...
  period.averagePeriodProductionCost =
    period.averagePeriodProductionCost / companies.length;
  period.averagePeriodUsingPower =
    period.averagePeriodUsingPower / companies.length;

  for (const player of companies) {
    const companyData: Company = player.getCompanyByPeriod(currentPeriodNumber);

    if (period.totalBuyers > 0) {
      companyData.marketingPart =
        (companyData.receivedOrders / period.totalBuyers) * 100;
    }
    ...
    const ratingByDemand =
      ((companyData.sumMarketing + companyData.sumNir) /
        (period.summaryMarketing + period.summaryNir)) *
      companiesAmount *
      scenario.ratingDemandEffect;

    const ratingBySupply =
      (companyData.sumProduction / period.summaryProduction) *
      companiesAmount *
      scenario.ratingSupplyEffect;

    const ratingByEfficiency =
      (1 - Math.abs(companyData.usingPower - 80) / 100) *
      scenario.ratingEfficiencyEffect;

    const ratingByMarketingPart = Math.min(
      (companyData.receivedOrders / period.totalBuyers) *
        companiesAmount *
        scenario.ratingMarketingPartEffect,
      20,
    );
    ...
    if (sales > 0 && period.totalSales > 0) {
      const salesPrevious = player.getCompanyByPeriod(currentPeriodNumber - 1)
        .sales;
      grow = salesPrevious
        ? ((scenario.ratingGrowEffect * sales) /
            salesPrevious /
            period.totalSales) *
    ...
    logger.info(
      `Game ${game.name}[${game.id}]: calculated ${player.companyName}: `,
      {
        companyData,
      },
    );
  ...
  logger.info(`Game ${game.name}[${game.id}]: calculated period: `, {
    period,
  });
