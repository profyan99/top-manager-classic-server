...
const removeGame = async (game: Game) => {
  logger.info(`Game ${game.name}[${game.id}]: removed`, {
    connectedPlayers: game.getConnectedPlayers(),
    actualPlayers: game.getActualPlayers(),
    existedPlayers: game.getExistedPlayers(),
    bankruptPlayers: game.getBankruptCount(),
  });
